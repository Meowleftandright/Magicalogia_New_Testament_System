...
export class TalentTableForm extends FormApplication {
    ...
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            ...
            template: "systems/magicalogia-new-testament/templates/talent-table-form.html",
            width: 500,
            closeOnSubmit: true,
    ...
    async _initTable() {
        ...
        for (var j = 0; j < 6; ++j) {
            ...
            var item = { title: await game.i18n.localize(`MAGICALOGIA.${name}${i}`), 
                         id: `col-${j}-${i}` ,
                         value: (this.reset) ? "" : game.settings.get("magicalogia-new-testament", `MAGICALOGIA.${name}${i}`)
                       }
    ...
    async _updateObject(event, formData) {
        ...
        for (var j = 0; j < 6; ++j) {
            let item = $(`#col-${j}-${i}`)[0];
            var name = String.fromCharCode(65 + j);
            game.settings.set("magicalogia-new-testament", `MAGICALOGIA.${name}${i}`, item.value);
        }
    }
