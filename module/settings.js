...
export class MagicalogiaSettings {
	static init() {
		game.settings.registerMenu("magicalogia-new-testament", "talentTable", {
		...
		game.settings.register("magicalogia-new-testament", "rollAddon", {
        ...
        for (var j = 0; j < 6; ++j) {			
			...
			game.settings.register("magicalogia-new-testament", `MAGICALOGIA.${name}${i}`, {
				scope: 'world',
				config: false,
				type: String,
				default: ""
		...
		Handlebars.registerHelper('localTalent', function(arg1, options) {
			let title = game.settings.get("magicalogia-new-testament", arg1);
		...
		Handlebars.registerHelper('mlgConcat', function() {
